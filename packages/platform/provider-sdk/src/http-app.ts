...
import type { PlatformError, ProviderExecutionRequest } from "../../contract-runtime/src/index.js";
import { createProviderTransportAuthenticator, newOpaqueId, ProviderProtocolError, type ProviderRuntime } from "./index.js";

export function buildProviderProtocolApp(runtime: ProviderRuntime, transportToken: string, readiness: () => Promise<{ ready: boolean; reasons: string[] }>): FastifyInstance {
  ...
  app.setErrorHandler((error, request, reply) => {
    const mapped = error instanceof ProviderProtocolError
      ? error
      : new ProviderProtocolError("INTERNAL_PROVIDER_ERROR", "provider execution failed", { cause: error });
