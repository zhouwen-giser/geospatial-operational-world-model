...
export function catalogRevisions(bindings: readonly CatalogBindingIdentity[], vocabularyHash = semanticVocabularyHash) {
  const ordered = [...bindings].sort((a,b) => a.manifest.provider.providerId.localeCompare(b.manifest.provider.providerId));
  const contracts = ordered.map(({ manifest }) => ({
    ...
    capabilities: [...manifest.capabilities].sort((a,b) => `${a.operationId}@${a.operationVersion}`.localeCompare(`${b.operationId}@${b.operationVersion}`)).map((c) => ({ operationId: c.operationId, operationVersion: c.operationVersion, contractHash: canonicalSha256(c) }))
  }));
