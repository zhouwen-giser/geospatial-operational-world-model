...
import { captureRuntimeSource } from "./runtime-source-fingerprint.mjs";
...
const originalRequired = matrix.filter((item) => item.required === "yes");
if (originalRequired.length !== 229 || new Set(originalRequired.map((item) => item.id)).size !== 229) {
  throw new Error(`expected 229 unique original rows, received ${originalRequired.length}`);
}
const amendmentPath = "validation/gowm-v0.6.1/current-design-amendment.json";
const amendment = JSON.parse(await readFile(resolve(root, amendmentPath), "utf8"));
if (JSON.stringify(Object.keys(amendment.superseded).sort()) !== JSON.stringify(["AC-R012", "AC-S-03"]) || amendment.effectiveRequiredCount !== 227) throw new Error("unapproved acceptance scope amendment");
const required = originalRequired.filter((item) => !(item.id in amendment.superseded));
const deliveryIds = new Set(["AC-S-22", "AC-S-23", "AC-S-24"]);
...
const capturedSource = await captureRuntimeSource(root);
const runtimePaths = Object.keys(capturedSource.files).sort();
for (const [name, report] of Object.entries(evidence)) {
  if (report.sourceBefore?.digest !== capturedSource.digest || report.sourceAfter?.digest !== capturedSource.digest) throw new Error(`${name} did not run on unchanged current source`);
}
...
assertChecks("gateway", evidence.gateway.summary?.checks, [
  ...
  "routingCurrentnessCurrent", "routingCurrentnessGraphStale", "routingCurrentnessTravelStale", "routingCurrentnessCostStale", "routingCurrentnessConditionStale", "routingCurrentnessUnavailable", "routingCurrentnessWorldStale", "routingValiditySeparate", "frozenPlanInvalid",
  "platformValidationSingleOwner", "platformRealRoutePublished", "platformRealPlanGraphChanged", "platformRoutingInputChanged", "platformUnknownResultSnapshot", "platformSiblingDatasetOpaque", "platformReferenceKinds", "worldGeometryAuthoritative", "resultGetCurrentSemantics", "resultGetSiblingDatasetOpaque",
...
if (conformance.status !== "PASS" || conformance.providers?.length !== 11 || !conformance.providers.every((provider) => provider.status === "PASS")) throw new Error("current-provider conformance evidence is incomplete");
if (conformance.sourceBefore?.digest !== capturedSource.digest || conformance.sourceAfter?.digest !== capturedSource.digest) throw new Error("conformance source fingerprint differs from current source");
const conformanceReports = new Map();
for (const slug of ["reference", "dataset-catalog", "world-evidence", "spatial", "h3", "h3-interactive", "stas", "network", "route", "road-coverage", "platform-validation"]) {
  const path = `reports/gowm-v0.6.1/provider-conformance/${slug}.json`;
  const report = JSON.parse(await readFile(resolve(root, path), "utf8"));
  verifyReportHash(report);
  if (report.status !== "PASS" || report.checks.some((check) => check.status !== "PASS") || !conformance.providers.some((entry) => entry.providerId === report.providerId && entry.reportHash === report.reportHash)) throw new Error(`invalid conformance evidence: ${slug}`);
  conformanceReports.set(slug, { ...report, path });
}
verifyReportHash(conformance);
...
if (Object.keys(caseMapping.cases ?? {}).length !== originalRequired.length) throw new Error("case mapping must retain all 229 original cases");
const runtimeChecks = {
  ...
  schema: {
    freshInstall: evidence.schema.summary.fresh.migrationCount === 58,
    deliberateRollback: evidence.schema.summary.deliberateFailureRollback === true,
    leastPrivilege: evidence.schema.summary.fresh.providerDirectMutation === false && evidence.schema.summary.fresh.providerNetworkMutation === false,
    currentAuthorityScopeAssertions: evidence.schema.summary.assertions === 43
  }
};
const cases = {};
for (const item of originalRequired) {
  if (item.id in amendment.superseded) { cases[item.id] = { status: "SUPERSEDED_BY_USER", scenario: item.scenario, reason: amendment.superseded[item.id], authority: amendmentPath }; continue; }
  ...
  const conformanceEvidence = (proof.conformance ?? []).map(({ provider, checks }) => {
    const report = conformanceReports.get(provider);
    if (!report || checks.length === 0 || checks.some((id) => !report.checks.some((c) => c.id === id && c.status === "PASS"))) throw new Error(`${item.id} has missing conformance checks for ${provider}`);
    return { provider, checks, path: report.path, reportHash: report.reportHash, status: "PASS" };
  });
  for (const path of proof.supportingEvidence) {
    const content = await readFile(resolve(root, path), "utf8");
    if (path.endsWith(".json") && JSON.parse(content).status !== "PASS") throw new Error(`${item.id}: supporting evidence is not PASS: ${path}`);
  }
  if (unitEvidence.length + runtimeEvidence.length + proof.supportingEvidence.length === 0) throw new Error(`${item.id} has no evidence`);
  cases[item.id] = { status: evidenceOnly && deliveryIds.has(item.id) ? "PENDING_DELIVERY" : "PASS", area: item.area, scenario: item.scenario, expected: item.expected, testType: item.testType, unitEvidence, runtimeEvidence, conformanceEvidence, supportingEvidence: proof.supportingEvidence };
}
if (evidenceOnly) {
  const output = "/tmp/gowm-v0.6.1-evidence-preflight.json";
  await writeFile(output, `${JSON.stringify({ decision: "EVIDENCE_PASS_DELIVERY_PENDING", originalCases: 229, effectiveRequiredCases: required.length, passedCases: required.length - deliveryIds.size, pendingDelivery: deliveryIds.size, supersededCases: 2, runtime: evidencePaths, testSummary, staticEvidence, cases }, null, 2)}\n`);
  process.stdout.write(`GOWM_V061_EVIDENCE_PASS required=${required.length} passed=${required.length - deliveryIds.size} superseded=2 delivery=PENDING output=${output}\n`);
  process.exit(0);
}
...
if (sync.status !== "AWAITING_DELIVERY_RECEIPT" || sync.phase !== "S03" || sync.acceptance.effectiveRequired !== 227 || sync.acceptance.superseded !== 2 || sync.acceptance.failed !== 0 || sync.acceptance.notRun !== 0 || sync.blockers.length !== 0) throw new Error("sync-state is incomplete");
...
for (const phase of phases) {
  ...
  if (!["PASS", "AWAITING_DELIVERY_RECEIPT"].includes(report.status) || report.requiredCases !== report.passedCases + (report.pendingDelivery ?? 0) || report.failedCases !== 0 || report.notRunCases !== 0) throw new Error(`${phase} phase acceptance is incomplete`);
}
...
const result = {
  ...
  originalRequiredCases: 229, requiredCases: required.length, passedCases: required.length, supersededCases: 2, scopeAmendment: amendmentPath, blockedCases: 0, failedCases: 0, notRunCases: 0,
...
process.stdout.write(`GOWM_V0_6_1_STABLE_CANDIDATE_COMPLETE required=${required.length} passed=${required.length} superseded=2 blocked=0 failed=0 notRun=0 sha=${local} output=${output}\n`);
...
function assertSchemaEvidence(report) {
  ...
  if (summary.migrations !== 58 || summary.assertions !== 43 || summary.fresh?.migrationCount !== 58 || summary.deliberateFailureRollback !== true) throw new Error("schema evidence does not prove current fresh install / assertions / rollback");
  if (summary.checksumReplaySkips?.[report.databases?.fresh] !== 58 || !report.cleanup?.every((item) => item.status === "PASS")) throw new Error("schema replay or cleanup evidence is incomplete");
}
async function runStaticGates() {
  ...
  for (const [command, args, label] of commands) {
    ...
    execFileSync(command, args, { cwd: root, encoding: "utf8", stdio: "pipe", maxBuffer: 128 * 1024 * 1024, env: { ...process.env, GOWM_CONFORMANCE_OUTPUT_DIRECTORY: "/tmp/gowm-v061-final-conformance" } });
  }
  const freshConformance = JSON.parse(await readFile("/tmp/gowm-v061-final-conformance/aggregate.json", "utf8"));
  if (freshConformance.reportHash !== conformance.reportHash) throw new Error("fresh conformance differs from the committed evidence");
...
function verifyReportHash(report) { const { reportHash, generatedAt, ...body } = report; if (reportHash !== `sha256:${createHash("sha256").update(canonical(body)).digest("hex")}`) throw new Error("Conformance report content hash mismatch"); }
