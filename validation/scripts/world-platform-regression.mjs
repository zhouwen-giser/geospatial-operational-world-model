...
const root=resolve('.'), reportRoot=process.env.GOWM_REPORT_DIRECTORY?.trim()||'reports/gowm-v0.6.2', dir=`${reportRoot}/regression`;
...
const suite=[
 ...
 [reportRoot.endsWith('gowm-v0.6.3')?'consumer-contracts':'wsgs-lock',['run',reportRoot.endsWith('gowm-v0.6.3')?'validate:consumer-contracts':'validate:wsgs-lock']],['provider-conformance',['run','validate:provider-conformance']]
];
for(const [id,args] of suite){
 const result=spawnSync('npm',args,{encoding:'utf8',maxBuffer:64*1024*1024,env:{...process.env,GOWM_REPORT_DIRECTORY:reportRoot,GOWM_CONFORMANCE_OUTPUT_DIRECTORY:resolve(`${reportRoot}/provider-conformance`)}});
