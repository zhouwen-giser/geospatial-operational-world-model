...
it("archives tracked deployment files without local trees or private environment files", async () => {
  ...
  try {
    const safe = ["package.json", "artifacts/h3-bindings.mjs", ".env.example",
      ".env.world-platform.example", "services/runtime/.env.example", "services/runtime/source file.ts"];
    const excluded = [".env", ".env.backup", "services/runtime/.env", "services/runtime/.env.local",
      ".worktrees/old/.env", ".worktrees/old/private.txt", ".pnpm-store/cache.txt", "task-packages/private.txt",
      "services/runtime/dist/old.js", "services/runtime/node_modules/module.js", "output/old.tar.gz",
      "reports/stale.json", "services/runtime/reports/stale.json", ".runtime/private.txt", "SHA256SUMS",
      "validation/probe.test.ts", "services/runtime/vitest.config.ts", "services/runtime/fixtures.ts",
      "services/runtime/examples/input.json", "services/example/source.ts", "tests/unit/spec.ts",
      "GOWM_Grounding_Operational_Stable_v0.4_Codex_Goal/21_TEST_ACCEPTANCE.md",
      "artifacts/opendrive-task-network-v0.1/unapproved.json"];
    ...
    await writeFile(join(fixture, "services/runtime/untracked-private.txt"), "synthetic untracked data\n");
