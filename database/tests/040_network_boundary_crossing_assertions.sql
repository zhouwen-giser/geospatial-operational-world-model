...
BEGIN
  IF to_regprocedure('gowm_network_v1.segment_boundary_crossings(uuid,jsonb,text,integer,integer)') IS NULL OR
     to_regprocedure('gowm_network_v1.route_boundary_crossings(uuid,jsonb,jsonb)') IS NULL OR
     to_regprocedure('gowm_network_v1.route_boundary_membership(uuid,jsonb,jsonb)') IS NULL THEN
  ...
  IF NOT has_function_privilege('coverage_planner_provider', 'gowm_network_v1.route_boundary_crossings(uuid,jsonb,jsonb)', 'EXECUTE') OR
     NOT has_function_privilege('route_planner_provider', 'gowm_network_v1.route_boundary_crossings(uuid,jsonb,jsonb)', 'EXECUTE') OR
     NOT has_function_privilege('coverage_planner_provider', 'gowm_network_v1.route_boundary_membership(uuid,jsonb,jsonb)', 'EXECUTE') THEN
