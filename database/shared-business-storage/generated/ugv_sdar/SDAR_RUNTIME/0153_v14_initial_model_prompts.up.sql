CREATE TEMPORARY TABLE initial_model_prompt_seeded ON COMMIT DROP AS WITH 
seed(prompt_id, stage) AS (VALUES
  ...
  inserted_prompts AS (INSERT INTO ugv_sdar.prompt (
    prompt_id,
    stage,
    current_version,
    created_at,
    ...
    clock_timestamp(),
    clock_timestamp()
  FROM seed ON CONFLICT DO NOTHING RETURNING prompt_id, stage, created_at)
SELECT
  prompt_id,
  stage,
  created_at
...
UPDATE ugv_sdar.prompt AS target SET current_version = 1,updated_at = seeded.created_at FROM initial_model_prompt_seeded AS seeded WHERE target.prompt_id = seeded.prompt_id;

INSERT INTO ugv_sdar.schema_migration (
  version
) VALUES
  ('0153_v14_initial_model_prompts');
