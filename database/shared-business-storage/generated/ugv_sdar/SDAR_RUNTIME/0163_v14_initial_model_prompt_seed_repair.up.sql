CREATE TEMPORARY TABLE initial_model_prompt_seed_repair ON COMMIT DROP AS WITH 
seed(prompt_id, stage) AS (VALUES
  ...
  inserted AS (INSERT INTO ugv_sdar.prompt (
    prompt_id,
    stage,
    current_version,
    created_at,
  ...
  WHERE
    NOT (EXISTS (SELECT 1
  FROM ugv_sdar.prompt AS existing
  WHERE
    existing.stage = seed.stage)) ON CONFLICT DO NOTHING RETURNING prompt_id, stage, created_at)
SELECT
  prompt_id,
  stage,
  created_at
...
INSERT INTO ugv_sdar.prompt_version (
  prompt_id,
  stage,
  version,
  previous_version,
...
UPDATE ugv_sdar.prompt AS target SET current_version = 1,updated_at = seeded.created_at FROM initial_model_prompt_seed_repair AS seeded WHERE target.prompt_id = seeded.prompt_id
...
INSERT INTO ugv_sdar.schema_migration (
  ...
  ('0163_v14_initial_model_prompt_seed_repair');
