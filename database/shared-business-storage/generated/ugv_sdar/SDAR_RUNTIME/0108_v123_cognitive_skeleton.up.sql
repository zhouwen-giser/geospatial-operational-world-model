CREATE TABLE ugv_sdar.cognitive_runtime_configuration (
...
CREATE TABLE ugv_sdar.runtime_capability_summary (
...
CREATE UNIQUE INDEX runtime_capability_summary_one_active ON ugv_sdar.runtime_capability_summary ((status)) WHERE status = 'active';

CREATE TABLE ugv_sdar.runtime_capability_summary_item (
  summary_id text NOT NULL REFERENCES ugv_sdar.runtime_capability_summary (summary_id)
    ON DELETE CASCADE,
...
CREATE TABLE ugv_sdar.runtime_capability_limitation (
  ...
  FOREIGN KEY(summary_id, capability_id)
    REFERENCES ugv_sdar.runtime_capability_summary_item (summary_id, capability_id)
    ON DELETE CASCADE
);

CREATE TABLE ugv_sdar.public_capability_card_snapshot (
...
CREATE UNIQUE INDEX public_capability_card_one_active ON ugv_sdar.public_capability_card_snapshot ((status)) WHERE status = 'active';

CREATE TABLE ugv_sdar.generic_task_understanding (
  understanding_id text PRIMARY KEY,
  task_id text NOT NULL,
  revision int NOT NULL CHECK (revision >= 1),
  disposition text NOT NULL CHECK (disposition IN ('clarification_required', 'confirmation_required', 'contract_candidate', 'rejected')),
...
CREATE TABLE ugv_sdar.generic_task_understanding_dimension (
  understanding_id text NOT NULL REFERENCES ugv_sdar.generic_task_understanding (understanding_id)
    ON DELETE CASCADE,
  dimension_id text NOT NULL,
  kind text NOT NULL CHECK (kind IN ('target', 'scope', 'time_range', 'criteria', 'artifact', 'evidence', 'side_effect_authorization')),
...
CREATE TABLE ugv_sdar.interactive_goal_session (
  session_id text PRIMARY KEY,
  task_id text NOT NULL UNIQUE,
  state text NOT NULL CHECK (state IN ('understand', 'goal_review', 'confirmed', 'rejected', 'canceled', 'budget_exhausted')),
  version int NOT NULL CHECK (version >= 1),
  current_understanding_id text REFERENCES ugv_sdar.generic_task_understanding (understanding_id),
  current_candidate_id text,
  current_candidate_revision int CHECK (current_candidate_revision >= 1),
  clarification_rounds int NOT NULL CHECK (clarification_rounds >= 0),
  revision_count int NOT NULL CHECK (revision_count >= 0),
  max_clarification_rounds int NOT NULL CHECK (max_clarification_rounds >= 0),
  max_revisions int NOT NULL CHECK (max_revisions >= 0),
  created_at timestamptz NOT NULL,
  updated_at timestamptz NOT NULL,
...
CREATE TABLE ugv_sdar.interactive_goal_turn (
  turn_id text PRIMARY KEY,
  session_id text NOT NULL REFERENCES ugv_sdar.interactive_goal_session (session_id)
    ON DELETE CASCADE,
  ordinal int NOT NULL CHECK (ordinal >= 1),
  expected_session_version int NOT NULL CHECK (expected_session_version >= 1),
...
CREATE TABLE ugv_sdar.goal_contract_candidate (
...
CREATE TABLE ugv_sdar.interactive_planning_session (
  session_id text PRIMARY KEY,
  task_id text NOT NULL UNIQUE,
  goal_session_id text NOT NULL REFERENCES ugv_sdar.interactive_goal_session (session_id),
  confirmed_contract_candidate_id text NOT NULL REFERENCES ugv_sdar.goal_contract_candidate (candidate_id),
...
CREATE TABLE ugv_sdar.interactive_planning_turn (
  turn_id text PRIMARY KEY,
  session_id text NOT NULL REFERENCES ugv_sdar.interactive_planning_session (session_id)
    ON DELETE CASCADE,
  ordinal int NOT NULL CHECK (ordinal >= 1),
  expected_session_version int NOT NULL CHECK (expected_session_version >= 1),
  idempotency_key text NOT NULL,
...
CREATE TABLE ugv_sdar.user_goal_plan_candidate (
  candidate_id text PRIMARY KEY,
  session_id text NOT NULL REFERENCES ugv_sdar.interactive_planning_session (session_id)
    ON DELETE CASCADE,
  revision int NOT NULL CHECK (revision >= 1),
...
CREATE TABLE ugv_sdar.planning_correction_fact (
...
CREATE TABLE ugv_sdar.planning_interaction_episode (
...
CREATE TABLE ugv_sdar.cognitive_runtime_outbox (
...
CREATE INDEX cognitive_runtime_outbox_unpublished ON ugv_sdar.cognitive_runtime_outbox (occurred_at, event_id) WHERE published_at IS NULL;

CREATE TABLE ugv_sdar.cognitive_runtime_consumer_cursor (
...
CREATE TABLE ugv_sdar.experience_job (
...
CREATE INDEX experience_job_claimable ON ugv_sdar.experience_job (status, available_at, job_id);

CREATE TABLE ugv_sdar.experience_dead_letter (
  dead_letter_id text PRIMARY KEY,
  job_id text NOT NULL UNIQUE REFERENCES ugv_sdar.experience_job (job_id),
...
CREATE TABLE ugv_sdar.goal_experience_episode (
  episode_id text PRIMARY KEY,
  goal_id text NOT NULL,
  goal_version int NOT NULL CHECK (goal_version >= 1),
  revision int NOT NULL CHECK (revision >= 1),
  episode_hash text NOT NULL CHECK (episode_hash ~ '^sha256:[0-9a-f]{64}$'),
  completeness numeric(5, 4) NOT NULL CHECK (completeness BETWEEN 0 AND 1),
...
CREATE TABLE ugv_sdar.goal_experience_episode_source (
  episode_id text NOT NULL REFERENCES ugv_sdar.goal_experience_episode (episode_id)
    ON DELETE CASCADE,
...
CREATE TABLE ugv_sdar.experience_observation (
...
CREATE TABLE ugv_sdar.experience_observation_fact (
  observation_id text NOT NULL REFERENCES ugv_sdar.experience_observation (observation_id)
    ON DELETE CASCADE,
...
CREATE TABLE ugv_sdar.experience_extraction (
...
CREATE TABLE ugv_sdar.experience_reflection (
...
CREATE TABLE ugv_sdar.planning_heuristic (
...
CREATE TABLE ugv_sdar.planning_heuristic_evidence (
  knowledge_id text NOT NULL,
  knowledge_revision int NOT NULL,
  ...
  FOREIGN KEY(knowledge_id, knowledge_revision)
    REFERENCES ugv_sdar.planning_heuristic (knowledge_id, revision)
    ON DELETE CASCADE
);

CREATE TABLE ugv_sdar.task_type_definition (
  knowledge_id text NOT NULL,
  revision int NOT NULL CHECK (revision >= 1),
  status text NOT NULL CHECK (status IN ('candidate', 'validating', 'active', 'deprecated', 'rejected')),
  scope text NOT NULL CHECK (scope IN ('task', 'user', 'tenant', 'global_candidate')),
...
CREATE TABLE ugv_sdar.task_type_evidence (
  knowledge_id text NOT NULL,
  knowledge_revision int NOT NULL,
  evidence_id text NOT NULL,
  polarity text NOT NULL CHECK (polarity IN ('support', 'contradiction')),
  ...
  PRIMARY KEY (knowledge_id, knowledge_revision, evidence_id),
  FOREIGN KEY(knowledge_id, knowledge_revision)
    REFERENCES ugv_sdar.task_type_definition (knowledge_id, revision)
    ON DELETE CASCADE
);

CREATE TABLE ugv_sdar.capability_pattern_definition (
  knowledge_id text NOT NULL,
  revision int NOT NULL CHECK (revision >= 1),
  status text NOT NULL CHECK (status IN ('candidate', 'validating', 'active', 'deprecated', 'rejected')),
  scope text NOT NULL CHECK (scope IN ('task', 'user', 'tenant', 'global_candidate')),
...
CREATE TABLE ugv_sdar.capability_pattern_evidence (
  knowledge_id text NOT NULL,
  knowledge_revision int NOT NULL,
  evidence_id text NOT NULL,
  polarity text NOT NULL CHECK (polarity IN ('support', 'contradiction')),
  ...
  PRIMARY KEY (knowledge_id, knowledge_revision, evidence_id),
  FOREIGN KEY(knowledge_id, knowledge_revision)
    REFERENCES ugv_sdar.capability_pattern_definition (knowledge_id, revision)
    ON DELETE CASCADE
);

CREATE TABLE ugv_sdar.capability_experience_evidence (
...
CREATE TABLE ugv_sdar.knowledge_promotion_evaluation (
  evaluation_id text PRIMARY KEY,
  knowledge_kind text NOT NULL CHECK (knowledge_kind IN ('planning_heuristic', 'task_type', 'capability_pattern')),
  knowledge_id text NOT NULL,
  knowledge_revision int NOT NULL CHECK (knowledge_revision >= 1),
  policy_version text NOT NULL,
...
CREATE TABLE ugv_sdar.knowledge_status_transition (
  transition_id text PRIMARY KEY,
  knowledge_kind text NOT NULL CHECK (knowledge_kind IN ('planning_heuristic', 'task_type', 'capability_pattern')),
  knowledge_id text NOT NULL,
  knowledge_revision int NOT NULL CHECK (knowledge_revision >= 1),
  expected_version int NOT NULL CHECK (expected_version >= 1),
...
CREATE TABLE ugv_sdar.experience_usage_record (
  usage_id text PRIMARY KEY,
  planning_session_id text NOT NULL REFERENCES ugv_sdar.interactive_planning_session (session_id),
  plan_candidate_id text NOT NULL REFERENCES ugv_sdar.user_goal_plan_candidate (candidate_id),
  knowledge_kind text NOT NULL CHECK (knowledge_kind IN ('planning_heuristic', 'task_type', 'capability_pattern')),
  knowledge_id text NOT NULL,
  knowledge_revision int NOT NULL CHECK (knowledge_revision >= 1),
