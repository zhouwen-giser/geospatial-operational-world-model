...
CREATE TABLE ugv_sdar.evidence_export_configuration (
...
CREATE UNIQUE INDEX evidence_export_one_active_idx ON ugv_sdar.evidence_export_configuration ((true)) WHERE is_active;

CREATE UNIQUE INDEX evidence_export_one_lkg_idx ON ugv_sdar.evidence_export_configuration ((true)) WHERE is_lkg;

CREATE TABLE ugv_sdar.evidence_export_state (
  export_id text NOT NULL,
  source_partition text NOT NULL,
  status text NOT NULL DEFAULT 'idle' CHECK (status IN ('idle', 'exporting', 'degraded', 'high_watermark', 'disabled')),
  last_sent_sequence bigint CHECK (
    last_sent_sequence IS NULL
      OR last_sent_sequence >= 0
  ),
  last_acknowledged_sequence bigint CHECK (
    last_acknowledged_sequence IS NULL
      OR last_acknowledged_sequence >= 0
  ),
  ...
  CHECK (
    last_sent_sequence IS NULL
      OR last_acknowledged_sequence IS NULL
...
CREATE TABLE ugv_sdar.evidence_outbox (
  sequence bigserial PRIMARY KEY,
  record_id text NOT NULL CHECK (record_id ~ '^evidence_[a-f0-9]{64}$'),
  record_family text NOT NULL CHECK (record_family IN ('runtime', 'skill', 'mcp_task', 'capability', 'experience', 'replay', 'artifact', 'node_control', 'evidence')),
  record_type text NOT NULL CHECK (record_type ~ E'^[a-z][a-z0-9_]*\\.[a-z][a-z0-9_]*$'),
  schema_name text NOT NULL CHECK (schema_name ~ E'^sdar\\.evidence\\.[a-z][a-z0-9_]*\\.[a-z][a-z0-9_]*$'),
  ...
  run_id text,
  goal_id text,
  goal_version int CHECK (
    goal_version IS NULL
      OR goal_version > 0
  ),
  plan_id text,
  plan_version int CHECK (
  ...
  causation_id text,
  delivery_guarantee text NOT NULL CHECK (delivery_guarantee IN ('transactional', 'durable_projection', 'buffered')),
  ...
  artifact_refs jsonb NOT NULL DEFAULT '[]'::jsonb CHECK (
    jsonb_typeof(artifact_refs) = 'array'
      AND pg_column_size(artifact_refs) <= 65536
  ),
  ...
  sent_export_id text,
  sent_fencing_token bigint CHECK (
    sent_fencing_token IS NULL
      OR sent_fencing_token >= 0
  ),
  sent_at timestamptz,
  ...
  CHECK (
    (sent_export_id IS NULL
      AND sent_fencing_token IS NULL
      AND sent_at IS NULL)
      OR (sent_export_id IS NOT NULL
      AND sent_fencing_token IS NOT NULL
      AND sent_at IS NOT NULL)
  )
);

CREATE INDEX evidence_outbox_pending_idx ON ugv_sdar.evidence_outbox (source_partition, next_attempt_at, sequence) WHERE acknowledged_at IS NULL;

CREATE INDEX evidence_outbox_episode_idx ON ugv_sdar.evidence_outbox (episode_id, sequence) WHERE episode_id IS NOT NULL;

CREATE TABLE ugv_sdar.evidence_source_checkpoint (
  source_family text NOT NULL CHECK (length(btrim(source_family)) > 0),
  source_partition text NOT NULL CHECK (length(btrim(source_partition)) > 0),
  last_occurred_at timestamptz,
  last_source_record_id text,
  last_source_revision text,
  last_payload_hash char(71) CHECK (
    last_payload_hash IS NULL
      OR last_payload_hash ~ '^sha256:[a-f0-9]{64}$'
  ),
  ...
  CHECK (
    last_source_record_id IS NOT NULL
      OR (last_occurred_at IS NULL
      AND last_payload_hash IS NULL
      AND last_projected_at IS NULL)
  )
);

CREATE TABLE ugv_sdar.evidence_dead_letter (
  dead_letter_id text PRIMARY KEY,
  sequence bigint NOT NULL UNIQUE REFERENCES ugv_sdar.evidence_outbox (sequence)
    ON DELETE RESTRICT,
  record_id text NOT NULL REFERENCES ugv_sdar.evidence_outbox (record_id)
    ON DELETE RESTRICT,
...
CREATE TABLE ugv_sdar.evidence_projection_issue (
  ...
  CHECK (
    evaluation_role <> 'required'
      OR severity IN ('degraded', 'blocking')
  )
);

CREATE INDEX evidence_projection_issue_open_idx ON ugv_sdar.evidence_projection_issue (source_system, source_partition, created_at) WHERE resolved_at IS NULL;

CREATE TABLE ugv_sdar.evidence_quality_issue (
  issue_id text PRIMARY KEY,
  issue_code text NOT NULL CHECK (issue_code IN ('schema_invalid', 'source_identity_missing', 'source_revision_missing', 'payload_hash_conflict', 'reference_unresolved', 'redaction_rejected', 'artifact_write_failed', 'export_rejected', 'ack_invalid', 'source_unavailable', 'projection_bug')),
  severity text NOT NULL CHECK (severity IN ('diagnostic', 'degraded', 'blocking')),
  record_type text,
...
CREATE INDEX evidence_quality_issue_episode_idx ON ugv_sdar.evidence_quality_issue (episode_id, created_at) WHERE episode_id IS NOT NULL;

CREATE TABLE ugv_sdar.episode_evidence_manifest (
  manifest_id text PRIMARY KEY,
  episode_id text NOT NULL UNIQUE,
  task_id text NOT NULL,
  terminal_outcome_id text NOT NULL,
  expected_required_records int NOT NULL CHECK (expected_required_records >= 0),
  projected_required_records int NOT NULL CHECK (projected_required_records >= 0),
  pending_required_records int NOT NULL CHECK (pending_required_records >= 0),
  failed_required_records int NOT NULL CHECK (failed_required_records >= 0),
  ...
  source_coverage jsonb NOT NULL CHECK (
    jsonb_typeof(source_coverage) = 'object'
      AND pg_column_size(source_coverage) <= 262144
  ),
  ...
  CHECK (
    status <> 'complete'
      OR (projected_required_records = expected_required_records
      AND pending_required_records = 0
      AND failed_required_records = 0
