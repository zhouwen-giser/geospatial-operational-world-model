...
CREATE TABLE ugv_sdar.remote_task_reconciliation_attempt (
  attempt_id text PRIMARY KEY CHECK (length(btrim(attempt_id)) BETWEEN 1 AND 256),
  intent_id text NOT NULL REFERENCES ugv_sdar.remote_task_admission_intent (intent_id)
    ON DELETE RESTRICT,
  logical_invocation_id text NOT NULL,
  expected_intent_version int NOT NULL CHECK (expected_intent_version > 0),
  ...
  FOREIGN KEY(logical_invocation_id)
    REFERENCES ugv_sdar.remote_task_admission_intent (logical_invocation_id)
    ON DELETE RESTRICT,
...
CREATE INDEX remote_task_reconciliation_pending_idx ON ugv_sdar.remote_task_admission_intent (updated_at, intent_id) WHERE status = 'uncertain'
  AND logical_invocation_id IS NOT NULL;

CREATE INDEX remote_task_reconciliation_attempt_intent_idx ON ugv_sdar.remote_task_reconciliation_attempt (intent_id, attempt_number);

CREATE TABLE ugv_sdar.remote_task_provider_execution_link (
  link_id text PRIMARY KEY CHECK (length(btrim(link_id)) BETWEEN 1 AND 256),
  binding_id text NOT NULL UNIQUE REFERENCES ugv_sdar.remote_task_binding (binding_id)
    ON DELETE RESTRICT,
  ...
  source_contract text NOT NULL CHECK (source_contract = 'sdar.node-control-provider-binding/v1+frozen-mcp-v1'),
  ...
  CHECK (
    mission_status <> 'exact'
      OR execution_status = 'exact'
  ),
  CHECK (
    (provider_origin_type IS NULL
      AND provider_binding_id IS NULL
      AND smpp_source_id IS NULL
      AND external_server_id IS NULL)
      OR (provider_origin_type = 'direct'
      AND provider_binding_id IS NOT NULL
      AND smpp_source_id IS NULL
      AND external_server_id IS NULL)
      OR (provider_origin_type = 'smpp_registry'
      AND provider_binding_id IS NOT NULL
...
CREATE FUNCTION ugv_sdar.remote_task_consumer_sync_immutable() RETURNS trigger LANGUAGE plpgsql AS $$
BEGIN
...
CREATE TRIGGER remote_task_reconciliation_attempt_immutable
  ...
  ON ugv_sdar.remote_task_reconciliation_attempt
  FOR EACH ROW
  EXECUTE PROCEDURE remote_task_consumer_sync_immutable();

CREATE TRIGGER remote_task_provider_execution_link_immutable
  BEFORE DELETE OR UPDATE
  ON ugv_sdar.remote_task_provider_execution_link
  FOR EACH ROW
  EXECUTE PROCEDURE remote_task_consumer_sync_immutable();
...
INSERT INTO ugv_sdar.schema_migration (
  ...
  ('0175_v14_mcp_task_consumer_sync');
