CREATE TABLE ugv_sdar.runtime_agent_card_exposure_snapshot (
  revision bigint NOT NULL REFERENCES ugv_sdar.runtime_agent_card_revision (revision),
  exposure_id text NOT NULL,
  exposure_version int NOT NULL CHECK (exposure_version >= 1),
...
CREATE FUNCTION ugv_sdar.prevent_runtime_agent_card_exposure_snapshot_mutation() RETURNS trigger LANGUAGE plpgsql AS $$
BEGIN
...
CREATE TRIGGER runtime_agent_card_exposure_snapshot_immutable
  ...
  ON ugv_sdar.runtime_agent_card_exposure_snapshot
  FOR EACH ROW
  EXECUTE PROCEDURE prevent_runtime_agent_card_exposure_snapshot_mutation();

CREATE TABLE ugv_sdar.task_capability_binding (
  binding_id text PRIMARY KEY,
  task_id text NOT NULL UNIQUE REFERENCES ugv_sdar.agent_task (task_id),
  requested_capability_id text NOT NULL,
  capability_version int NOT NULL CHECK (capability_version >= 1),
  exposure_id text,
  exposure_version int CHECK (exposure_version >= 1),
  input_snapshot jsonb NOT NULL,
  success_criteria_snapshot jsonb NOT NULL CHECK (
    jsonb_typeof(success_criteria_snapshot) = 'array'
      AND jsonb_array_length(success_criteria_snapshot) > 0
  ),
  ...
  initial_implementation_refs jsonb NOT NULL CHECK (
    jsonb_typeof(initial_implementation_refs) = 'array'
      AND jsonb_array_length(initial_implementation_refs) > 0
  ),
...
CREATE TABLE ugv_sdar.task_capability_execution_attempt (
  attempt_id text PRIMARY KEY,
  task_id text NOT NULL REFERENCES ugv_sdar.agent_task (task_id),
  capability_binding_id text NOT NULL REFERENCES ugv_sdar.task_capability_binding (binding_id),
  ...
  CHECK (
    (status = 'prepared'
      ...
      AND completed_at IS NULL)
      OR (status IN ('succeeded', 'failed', 'canceled', 'superseded')
      AND completed_at IS NOT NULL)
  )
);

CREATE INDEX task_capability_attempt_history_idx ON ugv_sdar.task_capability_execution_attempt (task_id, attempt_no);

CREATE FUNCTION ugv_sdar.prevent_task_capability_binding_mutation() RETURNS trigger LANGUAGE plpgsql AS $$
BEGIN
...
CREATE TRIGGER task_capability_binding_immutable
  BEFORE DELETE OR UPDATE
  ON ugv_sdar.task_capability_binding
  FOR EACH ROW
  EXECUTE PROCEDURE prevent_task_capability_binding_mutation();

CREATE FUNCTION ugv_sdar.protect_task_capability_attempt_content() RETURNS trigger LANGUAGE plpgsql AS $$
BEGIN
...
CREATE TRIGGER task_capability_attempt_content_immutable
  BEFORE UPDATE
  ON ugv_sdar.task_capability_execution_attempt
  FOR EACH ROW
  EXECUTE PROCEDURE protect_task_capability_attempt_content();

INSERT INTO ugv_sdar.schema_migration (
  version
) VALUES
  ('0139_v14_task_capability_binding');
