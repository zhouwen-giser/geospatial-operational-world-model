...
ALTER TABLE ugv_sdar.agent_task 
  ...
  ADD COLUMN IF NOT EXISTS command_lease_attempt int,
...
CREATE TABLE IF NOT EXISTS ugv_sdar.runtime_task_command_effect (
  action_id text NOT NULL REFERENCES ugv_sdar.cognitive_management_action (action_id)
    ON DELETE RESTRICT,
  ...
  CHECK (
    btrim(effect_kind) <> ''
      AND btrim(effect_ref) <> ''
  )
);

CREATE INDEX IF NOT EXISTS runtime_task_command_effect_task_idx ON ugv_sdar.runtime_task_command_effect (task_id, action_id, effect_kind);

ALTER TABLE ugv_sdar.agent_task 
  DROP CONSTRAINT IF EXISTS agent_task_revision_nonnegative RESTRICT,
  DROP CONSTRAINT IF EXISTS agent_task_command_identity_complete RESTRICT;

ALTER TABLE ugv_sdar.agent_task 
  ADD CONSTRAINT agent_task_revision_nonnegative 
    CHECK (revision >= 0),
  ADD CONSTRAINT agent_task_command_identity_complete 
    CHECK (
    (active_command_token IS NULL
      AND command_action_id IS NULL
      AND command_operation IS NULL
      AND command_idempotency_key IS NULL
      AND command_lease_attempt IS NULL
      AND command_lease_token IS NULL
      AND command_claimed_revision IS NULL
      AND command_precondition_json IS NULL
      AND command_claimed_at IS NULL
      AND command_execution_phase IS NULL
      AND command_result_json IS NULL
      AND command_completed_at IS NULL
      AND command_recovery_disposition IS NULL)
      OR (command_action_id IS NOT NULL
      AND btrim(command_action_id) <> ''
      AND command_operation IN ('pause', 'resume', 'cancel', 'goal-patch')
      AND command_idempotency_key IS NOT NULL
      AND btrim(command_idempotency_key) <> ''
      AND command_lease_attempt >= 1
      AND command_lease_token IS NOT NULL
      AND btrim(command_lease_token) <> ''
      AND command_claimed_revision >= 1
      AND command_precondition_json IS NOT NULL
      AND command_claimed_at IS NOT NULL
      AND ((active_command_token IS NOT NULL
      AND command_execution_phase IN ('claimed', 'dispatch_started')
      AND command_result_json IS NULL
      AND command_completed_at IS NULL
      AND command_recovery_disposition IS NULL)
      OR (active_command_token IS NULL
      AND command_execution_phase = 'completed'
      AND command_result_json IS NOT NULL
      AND command_completed_at IS NOT NULL
      AND command_recovery_disposition IS NULL)
      OR (active_command_token IS NULL
...
ALTER TABLE ugv_sdar.agent_task 
  VALIDATE CONSTRAINT agent_task_command_identity_complete;
...
CREATE OR REPLACE FUNCTION ugv_sdar.enforce_agent_task_revision_authority() RETURNS trigger LANGUAGE plpgsql AS $$
DECLARE
...
CREATE TRIGGER agent_task_revision_authority
  ...
  FOR EACH ROW
  EXECUTE PROCEDURE enforce_agent_task_revision_authority();

INSERT INTO ugv_sdar.schema_migration (
  version
) VALUES
  ('0162_v14_agent_task_revision_authority') ON CONFLICT (version) DO NOTHING;
