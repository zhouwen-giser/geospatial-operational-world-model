CREATE TABLE ugv_sdar.runtime_candidate_decision (
  decision_id text PRIMARY KEY,
  match_id text REFERENCES ugv_sdar.artifact_match_log (match_id)
    ON DELETE SET NULL,
  request_id text NOT NULL,
  path text NOT NULL CHECK (path IN ('compiled_fast', 'template_adapt', 'case_adapt', 'small_model', 'cognitive_runtime', 'human_input', 'denied')),
...
CREATE INDEX runtime_candidate_decision_request_idx ON ugv_sdar.runtime_candidate_decision (request_id, created_at DESC, decision_id);

INSERT INTO ugv_sdar.schema_migration (
  version
) VALUES
  ('0131_v13_artifact_retrieval_audit');
