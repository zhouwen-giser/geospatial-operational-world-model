...
UPDATE ugv_smpp.task_input_request SET request_json = jsonb_build_object('method', 'elicitation/create', 'params', jsonb_build_object('mode', 'form', 'message', description, 'requestedSchema', schema)),response_hash = CASE 
  WHEN status = 'ANSWERED'
    AND answer_hash IS NOT NULL THEN answer_hash 
  ELSE NULL 
END,response_json = CASE 
WHEN status = 'ANSWERED'
  AND answer_hash IS NOT NULL THEN jsonb_build_object('action', 'accept', 'content', answer) 
  ELSE NULL 
END;
