CREATE SEQUENCE IF NOT EXISTS ugv_smpp.ugv_diagnostic_fence_seq AS bigint;

CREATE TABLE IF NOT EXISTS ugv_smpp.ugv_diagnostic_lease (
...
CREATE INDEX IF NOT EXISTS ugv_diagnostic_lease_match_idx ON ugv_smpp.ugv_diagnostic_lease (capability_id, logical_invocation_id, state, fence);

CREATE INDEX IF NOT EXISTS ugv_diagnostic_lease_expiry_idx ON ugv_smpp.ugv_diagnostic_lease (expires_at) WHERE state IN ('ARMED', 'BOUND');

CREATE TABLE IF NOT EXISTS ugv_smpp.ugv_diagnostic_receipt (
  receipt_id uuid PRIMARY KEY,
  lease_id uuid NOT NULL REFERENCES ugv_smpp.ugv_diagnostic_lease (lease_id),
...
CREATE OR REPLACE FUNCTION ugv_smpp.reject_ugv_diagnostic_identity_mutation() RETURNS trigger LANGUAGE plpgsql AS $$
BEGIN
  ...
  RETURN NEW;
END;
$$ SET search_path TO ugv_smpp, public, pg_catalog;
...
CREATE TRIGGER ugv_diagnostic_identity_immutable
  BEFORE UPDATE
  ON ugv_smpp.ugv_diagnostic_lease
  FOR EACH ROW
  EXECUTE PROCEDURE reject_ugv_diagnostic_identity_mutation();

CREATE OR REPLACE FUNCTION ugv_smpp.reject_ugv_diagnostic_receipt_mutation() RETURNS trigger LANGUAGE plpgsql AS $$
BEGIN
...
CREATE TRIGGER ugv_diagnostic_receipt_append_only
  ...
  ON ugv_smpp.ugv_diagnostic_receipt
  FOR EACH ROW
  EXECUTE PROCEDURE reject_ugv_diagnostic_receipt_mutation();
