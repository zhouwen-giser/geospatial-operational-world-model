...
AS $fn$
  WITH trusted AS (
    ...
    SELECT DISTINCT ON (ranked.reference_key)
           ranked.reference_key, ranked.entity_kind, ranked.matched_by,
           ranked.match_score, ranked.match_priority
    FROM ranked
    ORDER BY ranked.reference_key, ranked.match_priority, ranked.match_score DESC, ranked.matched_by
  )
  ...
  ORDER BY deduplicated.match_priority, deduplicated.match_score DESC,
           deduplicated.reference_key
