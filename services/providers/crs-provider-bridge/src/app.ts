...
import type { CrsProviderBridge } from "./provider.js";

export function buildCrsProviderBridgeApp(bridge: CrsProviderBridge, transportToken: string): FastifyInstance {
  ...
  app.setErrorHandler((error, request, reply) => {
    const mapped = error instanceof ProviderProtocolError
      ? error
      : new ProviderProtocolError("INTERNAL_PROVIDER_ERROR", "CRS bridge execution failed", { cause: error });
