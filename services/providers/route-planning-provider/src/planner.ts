...
export class RoutePlanner {
  ...
  private async publish(id: string, generation: number, owner: string, output: Row): Promise<void> { const client = await this.options.pool.connect(); try { const query = await client.query("SELECT route_planner_runtime.publish_route_result($1::uuid,$2,$3,$4::jsonb,$5,$6,$7,$8) AS published", [id,generation,owner,JSON.stringify(output),sha256(output),sha256(output.routingSnapshot),"gowm-route-product-state/1.0.0","gowm-route-independent-verifier/1.1.0"]); if (query.rows[0]?.published!==true) throw new ProviderProtocolError("IDEMPOTENCY_CONFLICT","route publication lost its generation fence"); } finally { client.release(); } }
