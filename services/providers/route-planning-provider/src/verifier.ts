...
export function verifyRouteCandidate(network: LoadedNetwork, candidateValue: unknown, freshness?: RouteFreshness): Row {
  ...
  return { status: valid ? stale ? "STALE" : "VALID" : "INVALID", checks, verifierVersion: "gowm-route-independent-verifier/1.1.0", verifiedResultHash: sha256(candidate), warnings: stale ? [`The immutable route remains valid for its pinned snapshot; currentness is ${freshness?.currentness?.currentness ?? "STALE"}.`] : [] };
}

function replayTurn(history: string[], next: string, rules: TurnRule[]): { forbidden: boolean; penalty: number } { const candidate = [...history, next]; const previous = history.at(-1); const allowedOnly = rules.filter((rule) => rule.ruleType === "ALLOWED_ONLY" && rule.sequence.length === 2 && rule.sequence[0] === previous); if (allowedOnly.length > 0 && !allowedOnly.some((rule) => rule.sequence[1] === next)) return { forbidden: true, penalty: 0 }; let penalty = 0; for (const rule of rules) { if (rule.ruleType === "ALLOWED_ONLY" || rule.sequence.length > candidate.length) continue; const suffix = candidate.slice(-rule.sequence.length); if (!suffix.every((key, index) => key === rule.sequence[index])) continue; if (rule.ruleType === "FORBIDDEN") return { forbidden: true, penalty: 0 }; if (rule.ruleType === "PENALTY") penalty += rule.penaltyUnits; } return { forbidden: false, penalty }; }
