...
import type { GeometryProviderBridge } from "./provider.js";

export function buildGeometryProviderBridgeApp(bridge: GeometryProviderBridge, transportToken: string): FastifyInstance {
  ...
  app.get("/health/ready", async (_request, reply) => {
    const readiness = await bridge.upstream.readiness();
    return reply.code(readiness.ready ? 200 : 503).send({
      ...
      ...(readiness.upstream === undefined ? {} : { upstream: readiness.upstream })
    });
  });
  ...
  app.setErrorHandler((error, request, reply) => {
    const mapped = error instanceof ProviderProtocolError
      ? error
      : new ProviderProtocolError("INTERNAL_PROVIDER_ERROR", "Geometry bridge execution failed", { cause: error });
