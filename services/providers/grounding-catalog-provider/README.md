...
`GROUNDING_CATALOG_MODE=reference|dataset|result`. All modes use only their versioned
...
`gowm_catalog_service` for dataset mode, and `gowm_result_service` for result
registry mode.
