...
export function buildElevationMockApp(runtime: ProviderRuntime, transportToken: string): FastifyInstance {
  ...
  app.setErrorHandler((error, request, reply) => {
    const mapped = error instanceof ProviderProtocolError
      ? error
      : new ProviderProtocolError("INTERNAL_PROVIDER_ERROR", "elevation mock execution failed", { cause: error });
