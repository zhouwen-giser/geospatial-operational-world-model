...
import type { GowmSituationProvider } from "./provider.js";

export function buildGowmSituationProviderApp(provider: GowmSituationProvider, transportToken: string): FastifyInstance {
  ...
  app.get("/health/ready", async (_request, reply) => {
    const readiness = await provider.port.readiness?.() ?? { ready: true, reasons: [] };
  ...
  app.setErrorHandler((error, request, reply) => {
    const mapped = error instanceof ProviderProtocolError
      ? error
      : new ProviderProtocolError("INTERNAL_PROVIDER_ERROR", "GOWM Situation provider execution failed", { cause: error });
    const requestId = /^[A-Za-z][A-Za-z0-9._:-]{0,255}$/u.test(request.id) ? request.id : newOpaqueId("situation_request");
