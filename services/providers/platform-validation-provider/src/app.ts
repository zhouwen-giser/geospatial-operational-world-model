...
import type { PlatformValidationProvider } from "./index.js";

export function buildPlatformValidationApp(provider: PlatformValidationProvider, transportToken: string): FastifyInstance {
  ...
  app.get("/health/ready", async (_request, reply) => {
    const readiness = "readiness" in provider.authority && typeof provider.authority.readiness === "function"
      ? await provider.authority.readiness() as { ready: boolean; reasons: string[] }
      : { ready: true, reasons: [] };
    return reply.code(readiness.ready ? 200 : 503).send({ ...readiness, providerId: provider.runtime.manifest.provider.providerId, operationCount: provider.runtime.manifest.capabilities.length });
  });
  ...
  app.setErrorHandler((error, request, reply) => {
    const mapped = error instanceof ProviderProtocolError ? error : new ProviderProtocolError("INTERNAL_PROVIDER_ERROR", "platform validation execution failed", { cause: error });
    const requestId = /^[A-Za-z][A-Za-z0-9._:-]{0,255}$/u.test(request.id) ? request.id : newOpaqueId("validation_request");
    const payload: PlatformError = {
      schemaVersion: "1.0", requestId,
      error: { code: mapped.code, message: mapped.message, retryable: mapped.retryable, stage: ["INVALID_REQUEST", "SCHEMA_MISMATCH"].includes(mapped.code) ? "REQUEST_VALIDATION" : "PROVIDER_EXECUTION", providerId: provider.runtime.manifest.provider.providerId, ...(mapped.details === undefined ? {} : { details: { ...mapped.details } }) }
    };
