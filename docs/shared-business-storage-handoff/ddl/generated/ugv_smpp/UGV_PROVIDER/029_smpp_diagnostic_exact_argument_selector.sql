ALTER TABLE ugv_smpp.ugv_diagnostic_lease 
  ALTER COLUMN logical_invocation_id DROP NOT NULL,
  ADD COLUMN IF NOT EXISTS selector_argument_hash text;

ALTER TABLE ugv_smpp.ugv_diagnostic_lease 
  DROP CONSTRAINT IF EXISTS ugv_diagnostic_selector_argument_hash_valid RESTRICT;

ALTER TABLE ugv_smpp.ugv_diagnostic_lease 
  ADD CONSTRAINT ugv_diagnostic_selector_argument_hash_valid 
    CHECK (
    selector_argument_hash IS NULL
      OR selector_argument_hash ~ '^[0-9a-f]{64}$'
  );

DROP INDEX IF EXISTS ugv_diagnostic_lease_match_idx;

CREATE INDEX IF NOT EXISTS ugv_diagnostic_lease_selector_match_idx ON ugv_smpp.ugv_diagnostic_lease (selector_argument_hash, capability_id, state, fence) WHERE selector_argument_hash IS NOT NULL;
