CREATE TABLE ugv_smpp.provider_task (
  task_id uuid PRIMARY KEY,
  provider_id text NOT NULL,
  operation_name text NOT NULL,
  operation_snapshot_id uuid NOT NULL REFERENCES ugv_smpp.operation_snapshot (snapshot_id),
  ...
  internal_state text NOT NULL CHECK (internal_state IN ('SCHEDULED', 'STARTING', 'QUEUED', 'RUNNING', 'PAUSED', 'RESUMING', 'INPUT_REQUIRED', 'STOPPING', 'TERMINAL_COMPLETED', 'TERMINAL_FAILED', 'TERMINAL_CANCELLED')),
  ...
  poll_interval_ms int NOT NULL DEFAULT 2000 CHECK (poll_interval_ms >= 250),
...
CREATE TABLE ugv_smpp.admission_intent (
  task_id uuid PRIMARY KEY,
  provider_id text NOT NULL,
  operation_name text NOT NULL,
  operation_snapshot_id uuid NOT NULL REFERENCES ugv_smpp.operation_snapshot (snapshot_id),
  ...
  invocation_attempt int NOT NULL DEFAULT 1 CHECK (invocation_attempt > 0),
...
CREATE TABLE ugv_smpp.task_observation (
...
CREATE TABLE ugv_smpp.task_input_request (
  task_id uuid NOT NULL REFERENCES ugv_smpp.provider_task (task_id)
    ON DELETE CASCADE,
...
CREATE TABLE ugv_smpp.idempotency_record (
  authorization_context_hash char(64) NOT NULL,
  operation_name text NOT NULL,
  idempotency_key text NOT NULL,
  argument_hash char(64) NOT NULL,
  execution_mode text NOT NULL,
  task_id uuid REFERENCES ugv_smpp.provider_task (task_id),
...
CREATE TABLE ugv_smpp.outbox_event (
...
CREATE TABLE ugv_smpp.runtime_lease (
...
CREATE INDEX provider_task_nonterminal_idx ON ugv_smpp.provider_task (internal_state, updated_at) WHERE internal_state NOT LIKE 'TERMINAL_%';

CREATE INDEX provider_task_owner_idx ON ugv_smpp.provider_task (authorization_context_hash, execution_mode, task_id);

CREATE INDEX admission_intent_pending_idx ON ugv_smpp.admission_intent (state, updated_at) WHERE state IN ('PENDING', 'ACCEPTED', 'UNCERTAIN');

CREATE INDEX outbox_event_pending_idx ON ugv_smpp.outbox_event (created_at) WHERE published_at IS NULL;
