ALTER TABLE ugv_sdar.evidence_outbox 
  ADD COLUMN observation_generation smallint
    NOT NULL
    DEFAULT 0
    CHECK (observation_generation IN (0, 1));

CREATE SEQUENCE ugv_sdar.evidence_export_observation_sequence START 1;

CREATE TABLE ugv_sdar.evidence_export_batch (
  ledger_sequence bigint PRIMARY KEY DEFAULT nextval('evidence_export_observation_sequence'),
...
CREATE INDEX evidence_export_batch_cursor_idx ON ugv_sdar.evidence_export_batch (ledger_sequence, batch_id);

CREATE TABLE ugv_sdar.evidence_export_ack (
  ledger_sequence bigint PRIMARY KEY DEFAULT nextval('evidence_export_observation_sequence'),
  ack_id text NOT NULL UNIQUE,
  batch_id text NOT NULL,
  export_id text NOT NULL,
  source_partition text NOT NULL CHECK (length(btrim(source_partition)) > 0),
  acknowledged_sequence bigint CHECK (
    acknowledged_sequence IS NULL
      OR acknowledged_sequence >= 0
  ),
  batch_hash char(71) NOT NULL CHECK (batch_hash ~ '^sha256:[a-f0-9]{64}$'),
  ...
  FOREIGN KEY(batch_id, export_id, source_partition, batch_hash)
    REFERENCES ugv_sdar.evidence_export_batch (batch_id, export_id, source_partition, batch_hash),
  CHECK (
    (ack_disposition = 'rejected'
      AND acknowledged_sequence IS NULL
      AND error_code IS NOT NULL)
      OR (ack_disposition IN ('accepted', 'partial')
      AND acknowledged_sequence IS NOT NULL
      AND error_code IS NULL)
  ),
  UNIQUE (batch_id)
);

CREATE INDEX evidence_export_ack_cursor_idx ON ugv_sdar.evidence_export_ack (ledger_sequence, ack_id);

CREATE OR REPLACE FUNCTION ugv_sdar.reject_evidence_export_ledger_mutation() RETURNS trigger LANGUAGE plpgsql AS $$
BEGIN
...
CREATE TRIGGER evidence_export_batch_immutable
  ...
  ON ugv_sdar.evidence_export_batch
  FOR EACH ROW
  EXECUTE PROCEDURE reject_evidence_export_ledger_mutation();

CREATE TRIGGER evidence_export_ack_immutable
  BEFORE DELETE OR UPDATE
  ON ugv_sdar.evidence_export_ack
  FOR EACH ROW
  EXECUTE PROCEDURE reject_evidence_export_ledger_mutation();
...
INSERT INTO ugv_sdar.schema_migration (
  ...
  ('0146_v14_evidence_export_observation_ledger');
