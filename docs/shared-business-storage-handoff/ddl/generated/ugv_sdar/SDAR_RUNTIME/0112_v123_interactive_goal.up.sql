...
ALTER TABLE ugv_sdar.interactive_goal_session 
  ADD COLUMN max_elapsed_ms int
    NOT NULL
    CHECK (max_elapsed_ms > 0);

ALTER TABLE ugv_sdar.interactive_goal_turn 
  ADD COLUMN binding jsonb
    NOT NULL
    CHECK (jsonb_typeof(binding) = 'object');

ALTER TABLE ugv_sdar.goal_contract_candidate 
...
ALTER TABLE ugv_sdar.stage_model_route 
  ADD CONSTRAINT stage_model_route_stage_check 
    CHECK (stage IN ('intent', 'goal', 'goal_planning', 'tool_enhancement', 'skill_authoring', 'skill_selection', 'skill_input_resolution', 'workflow_planning', 'execution_decision', 'goal_evaluation', 'evaluation', 'result_processing', 'task_understanding', 'task_clarification', 'goal_contract_generation'));

INSERT INTO ugv_sdar.schema_migration (
  version
) VALUES
  ('0112_v123_interactive_goal');
