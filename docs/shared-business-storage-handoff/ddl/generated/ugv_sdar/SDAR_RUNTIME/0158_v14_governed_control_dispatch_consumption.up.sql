...
ALTER TABLE ugv_sdar.governed_control_confirmation 
  ...
  ADD COLUMN provider_binding_id text
    NOT NULL
    CHECK (length(btrim(provider_binding_id)) BETWEEN 1 AND 256),
  ...
  ADD CONSTRAINT governed_control_confirmation_tool_fk
    ...
    ON DELETE RESTRICT,
  ADD CONSTRAINT governed_control_confirmation_consumption_shape_check 
    CHECK (
    (consumed_invocation_id IS NULL
      AND consumed_dispatch_hash IS NULL
      AND consumed_at IS NULL)
      OR (consumed_invocation_id IS NOT NULL
  ...
  ADD CONSTRAINT governed_control_confirmation_consumed_dispatch_unique 
    UNIQUE (confirmation_id, task_id, capability_attempt_id, provider_binding_id, server_id, tool_name, arguments_hash, consumed_invocation_id, consumed_dispatch_hash);
...
ALTER TABLE ugv_sdar.mcp_invocation 
  ...
  ADD CONSTRAINT mcp_invocation_control_authority_shape_check 
    CHECK (
    (control_confirmation_id IS NULL
      ...
      OR (control_confirmation_id IS NOT NULL
  ...
  ADD CONSTRAINT mcp_invocation_control_authority_fk
    FOREIGN KEY(control_confirmation_id, task_id, capability_attempt_id, control_provider_binding_id, server_id, tool_name, control_arguments_hash, invocation_id, control_dispatch_hash)
    REFERENCES ugv_sdar.governed_control_confirmation (confirmation_id, task_id, capability_attempt_id, provider_binding_id, server_id, tool_name, arguments_hash, consumed_invocation_id, consumed_dispatch_hash)
    ON DELETE RESTRICT,
  ADD CONSTRAINT mcp_invocation_control_confirmation_unique 
    UNIQUE (control_confirmation_id);

CREATE OR REPLACE FUNCTION ugv_sdar.protect_governed_control_confirmation() RETURNS trigger LANGUAGE plpgsql AS $$
BEGIN
...
INSERT INTO ugv_sdar.schema_migration (
  ...
  ('0158_v14_governed_control_dispatch_consumption');
