CREATE TABLE ugv_sdar.artifact_replay_case (
  replay_case_id text PRIMARY KEY,
  tenant_id text NOT NULL,
  task_type_id text NOT NULL,
  primary_source_episode_id text NOT NULL REFERENCES ugv_sdar.goal_experience_episode (episode_id)
    ON DELETE CASCADE,
...
CREATE INDEX artifact_replay_case_scope_idx ON ugv_sdar.artifact_replay_case (tenant_id, task_type_id, created_at, replay_case_id);

CREATE TABLE ugv_sdar.artifact_replay_tenant_deletion (
  tenant_id text PRIMARY KEY,
  deleted_at timestamptz NOT NULL
);

CREATE TABLE ugv_sdar.replay_dataset_manifest (
  dataset_id text NOT NULL,
  dataset_version int NOT NULL CHECK (dataset_version >= 1),
  purpose text NOT NULL CHECK (purpose IN ('discovery', 'candidate_development', 'promotion_holdout', 'counterexample')),
  tenant_id text NOT NULL,
  content jsonb NOT NULL CHECK (
    jsonb_typeof(content) = 'object'
      AND octet_length(content::text) <= 1048576
  ...
  CHECK (
    (successor_dataset_id IS NULL
      AND successor_dataset_version IS NULL)
      OR (successor_dataset_id IS NOT NULL
      AND successor_dataset_version IS NOT NULL)
  ),
  PRIMARY KEY (dataset_id, dataset_version),
  UNIQUE (tenant_id, purpose, content_hash)
);

CREATE INDEX replay_dataset_manifest_scope_idx ON ugv_sdar.replay_dataset_manifest (tenant_id, purpose, created_at, dataset_id);

CREATE TABLE ugv_sdar.replay_dataset_case (
  dataset_id text NOT NULL,
  dataset_version int NOT NULL,
  replay_case_id text NOT NULL REFERENCES ugv_sdar.artifact_replay_case (replay_case_id)
    ON DELETE CASCADE,
  ...
  FOREIGN KEY(dataset_id, dataset_version)
    REFERENCES ugv_sdar.replay_dataset_manifest (dataset_id, dataset_version)
    ON DELETE CASCADE
);

ALTER TABLE ugv_sdar.artifact_validation_run 
  ADD COLUMN tenant_id text,
  ADD COLUMN dataset_version int,
  ...
  ADD COLUMN promotion_eligible boolean
    NOT NULL
    DEFAULT true,
  ...
  ADD COLUMN work_state text
    NOT NULL
    DEFAULT 'completed'
    CHECK (work_state IN ('pending', 'leased', 'retry_wait', 'completed', 'dead_letter', 'canceled')),
  ...
  ADD COLUMN last_error_summary text
    CHECK (
      last_error_summary IS NULL
        OR length(last_error_summary) BETWEEN 1 AND 2048
    ),
...
UPDATE ugv_sdar.artifact_validation_run SET idempotency_key = 'legacy:' || validation_run_id,available_at = started_at,created_at = started_at,updated_at = COALESCE(completed_at, started_at) WHERE idempotency_key IS NULL;
...
ALTER TABLE ugv_sdar.artifact_validation_run 
  ADD CONSTRAINT artifact_validation_run_replay_dataset_fk
    ...
    REFERENCES ugv_sdar.replay_dataset_manifest (dataset_id, dataset_version)
    ON DELETE RESTRICT,
  ADD CONSTRAINT artifact_validation_run_lease_check 
    CHECK (
    (work_state = 'leased') = (lease_owner IS NOT NULL
      AND lease_token IS NOT NULL
      AND lease_expires_at IS NOT NULL)
  ),
  ADD CONSTRAINT artifact_validation_run_replay_pin_check 
    CHECK (
...
CREATE UNIQUE INDEX artifact_validation_run_idempotency_idx ON ugv_sdar.artifact_validation_run (idempotency_key);

CREATE UNIQUE INDEX artifact_validation_run_source_event_idx ON ugv_sdar.artifact_validation_run (source_event_id) WHERE source_event_id IS NOT NULL;

CREATE INDEX artifact_validation_run_work_idx ON ugv_sdar.artifact_validation_run (work_state, available_at, validation_run_id);

CREATE INDEX artifact_validation_run_expired_lease_idx ON ugv_sdar.artifact_validation_run (lease_expires_at, validation_run_id) WHERE work_state = 'leased';

CREATE TABLE ugv_sdar.artifact_replay_case_result (
  validation_run_id text NOT NULL REFERENCES ugv_sdar.artifact_validation_run (validation_run_id)
    ON DELETE CASCADE,
...
CREATE TABLE ugv_sdar.artifact_validation_failure (
...
CREATE INDEX artifact_validation_failure_run_idx ON ugv_sdar.artifact_validation_failure (validation_run_id, severity, failure_id);

CREATE TABLE ugv_sdar.artifact_counterexample (
  counterexample_id text PRIMARY KEY,
  artifact_id text NOT NULL,
  artifact_version int NOT NULL,
  replay_case_id text NOT NULL,
  failure_id text NOT NULL REFERENCES ugv_sdar.artifact_validation_failure (failure_id)
    ON DELETE CASCADE,
  validation_run_id text NOT NULL REFERENCES ugv_sdar.artifact_validation_run (validation_run_id)
    ON DELETE CASCADE,
  content jsonb NOT NULL CHECK (
  ...
  condition_fingerprint text NOT NULL CHECK (condition_fingerprint ~ '^sha256:[0-9a-f]{64}$'),
...
CREATE INDEX artifact_counterexample_lineage_idx ON ugv_sdar.artifact_counterexample (artifact_id, artifact_version, replay_case_id);

CREATE FUNCTION ugv_sdar.sdar_reject_artifact_replay_content_mutation() RETURNS trigger LANGUAGE plpgsql AS $$
BEGIN
...
CREATE FUNCTION ugv_sdar.sdar_guard_replay_dataset_manifest_mutation() RETURNS trigger LANGUAGE plpgsql AS $$
BEGIN
...
CREATE FUNCTION ugv_sdar.sdar_guard_terminal_artifact_validation_run_mutation() RETURNS trigger LANGUAGE plpgsql AS $$
BEGIN
...
CREATE TRIGGER artifact_replay_case_immutability
  BEFORE UPDATE
  ON ugv_sdar.artifact_replay_case
  FOR EACH ROW
  EXECUTE PROCEDURE sdar_reject_artifact_replay_content_mutation();

CREATE TRIGGER replay_dataset_manifest_immutability
  BEFORE UPDATE
  ON ugv_sdar.replay_dataset_manifest
  FOR EACH ROW
  EXECUTE PROCEDURE sdar_guard_replay_dataset_manifest_mutation();

CREATE TRIGGER artifact_validation_run_terminal_immutability
  BEFORE UPDATE
  ON ugv_sdar.artifact_validation_run
  FOR EACH ROW
  EXECUTE PROCEDURE sdar_guard_terminal_artifact_validation_run_mutation();

CREATE TRIGGER artifact_replay_case_result_immutability
  BEFORE UPDATE
  ON ugv_sdar.artifact_replay_case_result
  FOR EACH ROW
  EXECUTE PROCEDURE sdar_reject_artifact_replay_content_mutation();

CREATE TRIGGER artifact_validation_failure_immutability
  BEFORE UPDATE
  ON ugv_sdar.artifact_validation_failure
  FOR EACH ROW
  EXECUTE PROCEDURE sdar_reject_artifact_replay_content_mutation();

CREATE TRIGGER artifact_counterexample_immutability
  BEFORE UPDATE
  ON ugv_sdar.artifact_counterexample
  FOR EACH ROW
  EXECUTE PROCEDURE sdar_reject_artifact_replay_content_mutation();

CREATE FUNCTION ugv_sdar.sdar_invalidate_replay_datasets_for_case() RETURNS trigger LANGUAGE plpgsql AS $$
DECLARE
  affected record;
...
CREATE TRIGGER artifact_replay_case_delete_propagation
  ...
  ON ugv_sdar.artifact_replay_case
  FOR EACH ROW
  EXECUTE PROCEDURE sdar_invalidate_replay_datasets_for_case();

INSERT INTO ugv_sdar.schema_migration (
  version
) VALUES
  ('0129_v13_artifact_replay_validation');
