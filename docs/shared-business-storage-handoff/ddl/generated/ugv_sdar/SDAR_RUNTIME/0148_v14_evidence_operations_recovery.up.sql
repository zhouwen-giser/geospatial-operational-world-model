ALTER TABLE ugv_sdar.evidence_dead_letter 
  ADD COLUMN requeue_count int
    NOT NULL
    DEFAULT 0
...
UPDATE ugv_sdar.evidence_dead_letter SET requeue_count = 1,requeued_by = 'pre-0148',requeue_reason = 'legacy requeue metadata unavailable' WHERE requeued_at IS NOT NULL;

ALTER TABLE ugv_sdar.evidence_dead_letter 
  ADD CONSTRAINT evidence_dead_letter_requeue_metadata_ck 
    CHECK (
    (requeued_at IS NULL
      AND requeued_by IS NULL
      AND requeue_reason IS NULL)
      OR (requeued_at IS NOT NULL
      AND length(btrim(requeued_by)) BETWEEN 1 AND 256
      AND length(btrim(requeue_reason)) BETWEEN 1 AND 2048
      AND requeue_count > 0)
  );

CREATE TABLE ugv_sdar.evidence_recovery_run (
  ...
  idempotency_key_hash char(71) NOT NULL UNIQUE CHECK (idempotency_key_hash ~ '^sha256:[a-f0-9]{64}$'),
  ...
  operation text NOT NULL CHECK (operation IN ('replay_record', 'replay_source_partition', 'replay_episode', 'retry_dead_letter', 'reconcile_coverage', 'apply_retention')),
  target jsonb NOT NULL CHECK (
    jsonb_typeof(target) = 'object'
      AND pg_column_size(target) <= 8192
  ),
  ...
  result_summary jsonb CHECK (
    result_summary IS NULL
      ...
      AND pg_column_size(result_summary) <= 65536)
  ),
...
CREATE INDEX evidence_recovery_run_pending_idx ON ugv_sdar.evidence_recovery_run (requested_at, recovery_run_id) WHERE status IN ('requested', 'running');

CREATE INDEX evidence_recovery_run_target_idx ON ugv_sdar.evidence_recovery_run (operation, requested_at DESC, recovery_run_id);

CREATE TABLE ugv_sdar.evidence_coverage_reconcile_target (
  recovery_run_id text NOT NULL REFERENCES ugv_sdar.evidence_recovery_run (recovery_run_id)
    ON DELETE RESTRICT,
...
CREATE INDEX evidence_coverage_reconcile_target_pending_idx ON ugv_sdar.evidence_coverage_reconcile_target (episode_id, requested_at, recovery_run_id) WHERE completed_at IS NULL;

INSERT INTO ugv_sdar.schema_migration (
  version
) VALUES
  ('0148_v14_evidence_operations_recovery');
