ALTER TABLE ugv_sdar.capability_pattern_definition 
  ...
  ADD COLUMN definition_origin text
    ...
    CHECK (definition_origin IN ('reflection_candidate', 'capability_pattern_induction', 'fixture')),
  ADD COLUMN model_invocation_id text
    REFERENCES ugv_sdar.model_invocation (invocation_id);

CREATE INDEX capability_pattern_definition_identity_idx ON ugv_sdar.capability_pattern_definition (capability_id, revision DESC, knowledge_id) WHERE capability_id IS NOT NULL;

CREATE INDEX capability_pattern_definition_active_catalog_idx ON ugv_sdar.capability_pattern_definition (status, catalog_hash, capability_id) WHERE status = 'active';

CREATE TABLE ugv_sdar.capability_gap_candidate (
  ...
  pattern_revision int NOT NULL CHECK (pattern_revision >= 1),
...
CREATE INDEX capability_gap_candidate_capability_idx ON ugv_sdar.capability_gap_candidate (capability_id, created_at DESC);
