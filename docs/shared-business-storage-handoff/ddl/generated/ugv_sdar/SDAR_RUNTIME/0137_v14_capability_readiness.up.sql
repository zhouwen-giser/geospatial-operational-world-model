CREATE TABLE ugv_sdar.capability_readiness_snapshot (
  ...
  candidate_status text CHECK (
    candidate_status IS NULL
      OR candidate_status IN ('available', 'degraded', 'unavailable', 'suspended')
  ),
...
CREATE INDEX capability_readiness_latest_idx ON ugv_sdar.capability_readiness_snapshot (capability_id, capability_version, snapshot_version DESC);

CREATE INDEX capability_readiness_expiry_idx ON ugv_sdar.capability_readiness_snapshot (valid_until, capability_id, capability_version, snapshot_version DESC);

CREATE TABLE ugv_sdar.capability_readiness_command_receipt (
  ...
  FOREIGN KEY(capability_id, capability_version, snapshot_version)
    REFERENCES ugv_sdar.capability_readiness_snapshot (capability_id, capability_version, snapshot_version)
);

CREATE FUNCTION ugv_sdar.prevent_capability_readiness_mutation() RETURNS trigger LANGUAGE plpgsql AS $$
BEGIN
...
CREATE TRIGGER capability_readiness_immutable
  ...
  ON ugv_sdar.capability_readiness_snapshot
  FOR EACH ROW
  EXECUTE PROCEDURE prevent_capability_readiness_mutation();

INSERT INTO ugv_sdar.schema_migration (
  version
) VALUES
  ('0137_v14_capability_readiness');
