CREATE TABLE ugv_sdar.governed_control_confirmation (
  ...
  capability_version int NOT NULL CHECK (capability_version > 0),
  plan_id text NOT NULL REFERENCES ugv_sdar.workflow_plan (plan_id)
    ON DELETE RESTRICT,
  ...
  authentication_method text NOT NULL CHECK (
    length(btrim(authentication_method)) BETWEEN 1 AND 128
  ...
  CHECK (
    expires_at > confirmed_at
      AND expires_at <= (confirmed_at + '15 minutes'::interval)
  ),
  CHECK ((revoked_at IS NULL) = (revoked_by IS NULL)),
  CHECK (
    revoked_at IS NULL
      OR revoked_at >= confirmed_at
  )
);

CREATE INDEX governed_control_confirmation_scope_idx ON ugv_sdar.governed_control_confirmation (task_id, capability_binding_id, plan_id, skill_id, skill_version, confirmed_at DESC);

CREATE FUNCTION ugv_sdar.protect_governed_control_confirmation() RETURNS trigger LANGUAGE plpgsql AS $$
BEGIN
...
CREATE TRIGGER governed_control_confirmation_immutable
  ...
  ON ugv_sdar.governed_control_confirmation
  FOR EACH ROW
  EXECUTE PROCEDURE protect_governed_control_confirmation();

INSERT INTO ugv_sdar.schema_migration (
  version
) VALUES
  ('0157_v14_governed_control_confirmation') ON CONFLICT (version) DO NOTHING;
