CREATE TABLE ugv_sdar.runtime_model_provider_catalog (
...
CREATE UNIQUE INDEX runtime_model_provider_one_active_idx ON ugv_sdar.runtime_model_provider_catalog (provider_id) WHERE is_active;

CREATE TABLE ugv_sdar.runtime_model_route_snapshot (
  configuration_id text NOT NULL,
  revision bigint NOT NULL CHECK (revision > 0),
  ...
  CHECK (
    (scope_type = 'stage'
      AND scope_key = '')
      OR (scope_type IN ('task', 'case')
      AND scope_key <> '')
  )
);

CREATE UNIQUE INDEX runtime_model_route_one_active_scope_idx ON ugv_sdar.runtime_model_route_snapshot (stage, scope_type, scope_key) WHERE is_active;

CREATE TABLE ugv_sdar.runtime_task_model_route_binding (
  ...
  FOREIGN KEY(route_configuration_id, route_revision)
    REFERENCES ugv_sdar.runtime_model_route_snapshot (configuration_id, revision)
);

CREATE OR REPLACE FUNCTION ugv_sdar.protect_runtime_task_model_route_binding() RETURNS trigger LANGUAGE plpgsql AS $$
BEGIN
...
CREATE TRIGGER runtime_task_model_route_binding_immutable
  ...
  ON ugv_sdar.runtime_task_model_route_binding
  FOR EACH ROW
  EXECUTE PROCEDURE protect_runtime_task_model_route_binding();

INSERT INTO ugv_sdar.schema_migration (
  version
) VALUES
  ('0136_v14_model_control_governance') ON CONFLICT (version) DO NOTHING;
