CREATE TABLE ugv_sdar.runtime_agent_card_revision (
...
CREATE UNIQUE INDEX runtime_agent_card_single_active_idx ON ugv_sdar.runtime_agent_card_revision ((status)) WHERE status = 'active';

CREATE TABLE ugv_sdar.runtime_agent_card_command_receipt (
  scope text NOT NULL,
...
CREATE FUNCTION ugv_sdar.prevent_runtime_agent_card_content_mutation() RETURNS trigger LANGUAGE plpgsql AS $$
BEGIN
...
CREATE TRIGGER runtime_agent_card_content_immutable
  BEFORE UPDATE
  ON ugv_sdar.runtime_agent_card_revision
  FOR EACH ROW
  EXECUTE PROCEDURE prevent_runtime_agent_card_content_mutation();

INSERT INTO ugv_sdar.schema_migration (
  version
) VALUES
  ('0138_v14_runtime_agent_card');
