CREATE TABLE ugv_sdar.case_runtime_match (
...
CREATE INDEX case_runtime_match_tenant_task_idx ON ugv_sdar.case_runtime_match (tenant_id, task_type_id, created_at DESC);

CREATE TABLE ugv_sdar.case_runtime_adaptation (
...
CREATE INDEX case_runtime_adaptation_case_idx ON ugv_sdar.case_runtime_adaptation (case_ref, created_at DESC);

CREATE TABLE ugv_sdar.model_route_decision (
...
CREATE INDEX model_route_decision_tenant_request_idx ON ugv_sdar.model_route_decision (tenant_id, request_ref, created_at DESC);

CREATE TABLE ugv_sdar.model_cascade_run (
  cascade_run_id text PRIMARY KEY,
  route_decision_ref text NOT NULL REFERENCES ugv_sdar.model_route_decision (route_decision_ref),
  decision_hash text NOT NULL CHECK (decision_hash ~ '^sha256:[0-9a-f]{64}$'),
  ...
  status text NOT NULL CHECK (status IN ('running', 'completed', 'fallback', 'cancelled', 'timed_out', 'budget_exhausted', 'failed')),
  completed_at timestamptz
);

CREATE INDEX model_cascade_run_route_idx ON ugv_sdar.model_cascade_run (route_decision_ref, cascade_run_id);

CREATE TABLE ugv_sdar.model_cascade_step (
  step_ref text PRIMARY KEY,
  cascade_run_id text NOT NULL REFERENCES ugv_sdar.model_cascade_run (cascade_run_id)
    ON DELETE CASCADE,
  profile_ref text NOT NULL,
  attempt int NOT NULL CHECK (attempt BETWEEN 1 AND 16),
...
CREATE INDEX model_cascade_step_run_idx ON ugv_sdar.model_cascade_step (cascade_run_id, step_ref);

INSERT INTO ugv_sdar.schema_migration (
  version
) VALUES
  ('0133_v13_case_model_runtime');
