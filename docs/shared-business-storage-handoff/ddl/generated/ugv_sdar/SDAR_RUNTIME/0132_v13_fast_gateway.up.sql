CREATE TABLE ugv_sdar.fast_gateway_request (
...
CREATE INDEX fast_gateway_request_task_idx ON ugv_sdar.fast_gateway_request (task_id, created_at DESC, request_id);

CREATE TABLE ugv_sdar.fast_gateway_decision (
  gateway_decision_id text PRIMARY KEY,
  request_id text NOT NULL UNIQUE REFERENCES ugv_sdar.fast_gateway_request (request_id)
    ON DELETE CASCADE,
...
CREATE TABLE ugv_sdar.fast_gateway_feedback (
  feedback_id text PRIMARY KEY,
  request_id text NOT NULL REFERENCES ugv_sdar.fast_gateway_request (request_id)
    ON DELETE CASCADE,
  gateway_decision_id text NOT NULL REFERENCES ugv_sdar.fast_gateway_decision (gateway_decision_id)
    ON DELETE CASCADE,
  feedback_type text NOT NULL CHECK (feedback_type IN ('route_selected', 'fallback', 'confirmation', 'denial', 'formal_handoff', 'outcome', 'correction', 'recovery', 'performance', 'drift')),
...
CREATE INDEX fast_gateway_feedback_decision_idx ON ugv_sdar.fast_gateway_feedback (gateway_decision_id, created_at, feedback_id);

INSERT INTO ugv_sdar.schema_migration (
  version
) VALUES
  ('0132_v13_fast_gateway');
